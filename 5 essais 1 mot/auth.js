    ...
    motArray.forEach((lettre, i) => {
      ...
      if (motEssayéArray[i] !== "") {
        if (motArray.includes(motEssayéArray[i])) {
          ...
          document.getElementById("C" + currentRow + i).style.backgroundColor =
            "yellow"; // Changer la couleur de la cellule de la lettre
...
function saveData(R) {
  // fonction qui enregistre les données de l'utilisateur
  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      partieTerminée: partieTerminée,
      balance: balance + 10 * R,
    })
  );
  sessionStorage.setItem('session','ouvert')
}

function fin() {
  // fonction pour le message à la fin de la partie
  console.log(
